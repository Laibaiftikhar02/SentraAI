...
def login_student():
    ...
    password = _env_value("SEED_USER_PASSWORD")
    if not password:
        raise RuntimeError(
            "SEED_USER_PASSWORD not found in environment. "
            "Ensure it is set in backend/.env"
        )
    status, data = req(
        "POST",
        "/api/v1/auth/login",
        {"email": "student1@sentraai.dev", "password": password},
    )
...
def main():
    """Run the live Gemini triage integration test."""
    global PASS, FAIL
...
def test_gemini_triage_live():
    """pytest entry point for the live Gemini triage integration test."""
    exit_code = main()
    assert exit_code == 0, f"Gemini triage live test failed ({FAIL} failures)"
